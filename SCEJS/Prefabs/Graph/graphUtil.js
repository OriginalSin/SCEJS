var adjMatrix_ForceLayout_GLSLFunctionString = function(geometryLength) {
    ...
    'CalculationResponse calculate(vec4 it, vec2 xGeom_oppo, vec3 currentPos, vec3 currentDir, vec3 atraction, float acumAtraction, vec3 repulsion) {'+
        'float radius = 4.0;\n'+
    ...
    '}'+
    'vec4 idAdjMatrix_ForceLayout(float nodeId, vec3 currentPos, vec3 currentDir, float numOfConnections, float currentTimestamp, float bornDate, float dieDate) {\n'+
        // INIT VARS
        ...
        'if(nodeId >= initCol && nodeId < (initCol+widthAdjMatrix)) {\n'+
            ...
            'for(int n=0; n < 4096; n++) {\n'+
                ...
                'if(idb != nodeId) {'+
                    ...
                    'CalculationResponse calcResponse = calculate(it, xGeom_oppo, currentPos, currentDir, atraction, acumAtraction, repulsion);'+
        ...
        'return vec4(force, collisionExists);'+
    '}';

    return str;
};

var adjMatrix_NeuronalNetwork_Efference_GLSLFunctionString = function(geometryLength) {
    var str = ''+
    'vec3 sphericalColl(vec3 currentDir, vec3 currentDirB, vec3 dirToBN) {'+
        'vec3 currentDirN = normalize(currentDir);'+
        'float pPoint = abs(dot(currentDirN, dirToBN));'+
    ...
    'CalculationResponse calculate(vec4 it, vec2 xGeom_oppo, vec3 currentPos, vec3 currentDir, vec3 atraction, float acumAtraction, vec3 repulsion) {'+
        ...
        'return CalculationResponse(atraction, acumAtraction, repulsion, collisionExists);'+
    '}'+
    'vec4 idAdjMatrix_NeuronalNetwork_Efference(float nodeId, vec3 currentPos, vec3 currentDir, float numOfConnections, float currentTimestamp, float bornDate, float dieDate) {\n'+
        // INIT VARS
        'vec3 atraction = vec3(0.0, 0.0, 0.0);'+
        'float acumAtraction = 1.0;'+
        'vec3 repulsion = vec3(0.0, 0.0, 0.0);'+
