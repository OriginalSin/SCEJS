...
UIComponentRenderer = function(compTypeKey, selectedNode) {
		...
		for(var argKey in args) {
			...
			for(var n=0, fn=kernel.kernel.in_values.length; n < fn; n++) {
				...
				if(fv.name == argKey) {
					...
					var isDest = (kernel.argBufferDestination == argKey) ? "<div style='background-color:rgba(50,50,150,1.0);color:rgba(0,0,0,0);width:20px;margin:0px auto 0px auto'>-</div>" : "";
					str += 	"<div style='background:"+bg+";color:rgba(0,0,0,0);'>-"+isDest+"</div>";
					exists = true;
					break;
				}
			}
	...
	for(var vfpKey in comp.getVFPs()) {
		...
		for(var argKey in args) {
			...
			for(var n=0, fn=vfp.vfp.in_vertex_values.length; n < fn; n++) {
				...
				if(vv.name == argKey) {
					...
					var isFromKernel = (arg.type == "buffer_float4_fromKernel") ? "<div style='background-color:rgba(50,50,150,1.0);color:rgba(0,0,0,0);width:20px;margin:0px auto 0px 0px'>-</div>" : "";
		...
		e.addEventListener("click", (function(comp, vfpKey, e) {
			if(e.checked == false) {
				comp.setEnableDepthTest(vfpKey, false);
			} else {
				comp.setEnableDepthTest(vfpKey, true);
			}
		}).bind(this, comp, vfpKey, e));
		...
		e.addEventListener("click", (function(comp, vfpKey, e) {
			if(e.checked == false) {
				comp.setEnableBlend(vfpKey, false);
			} else {
				comp.setEnableBlend(vfpKey, true);
			}
		}).bind(this, comp, vfpKey, e));
		...
		e.addEventListener("change", (function(comp, vfpKey, e) {
			comp.setBlendEquation(vfpKey, e.options[e.selectedIndex].value);
		}).bind(this, comp, vfpKey, e));
